...
def fetch_rss(url: str, source_name: str, default_category: str):
...
def fetch_all(config_path="config/sources.yaml"):
    ...
    for src in sources:
        try:
            if src["type"] == "rss":
                items = fetch_rss(src["url"], src["name"], src.get("default_category", "actualidad"))
            elif src["type"] == "gnews":
                feed_url = _google_news_rss_url(src["query"])
                items = fetch_rss(feed_url, src["name"], src.get("default_category", "actualidad"))
            else:
                continue
